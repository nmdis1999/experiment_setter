...
if [ -d "profiles" ]; then
    read -p "The 'profiles' directory already exists. Do you want to continue? (y/n) " answer
    if [ "$answer" = "n" ]; then
        echo "Directory profiles already exists, exiting script."
        exit 1
    fi
else
    mkdir profiles
...
ProfPath=$(pwd)/profles

read -p "Collecting profile for No LBR clang build in directory $(pwd)/profiles. Continue (y/n)?"
...
cd tmux && perf record -o $ProfPath/nolbr.data -- $CPATH/clang-16 -DPACKAGE_NAME=\"tmux\"
